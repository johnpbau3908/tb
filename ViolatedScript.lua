...
local Config = {
    ...
    TriggerDelay   = 0.05,
    FireRate       = 0.08,
    TeamCheck      = false, -- OFF by default (FFA game)
    KnockCheck     = true,
    KnifeCheck     = false,
}
...
-- ─── Target Detection (detects ANY part of enemy) ────────────────────────────

local function GetTarget()
    local target = Mouse.Target
    if not target then return nil end

    -- Walk up to find a Model with a Humanoid
    ...
    if Config.TeamCheck then
        if LocalPlayer.Team ~= nil and targetPlayer.Team == LocalPlayer.Team then return nil end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if target and not firing and (now - lastShot) >= Config.FireRate then
        firing = true
